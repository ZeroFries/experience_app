class BaseRepository
  ...
  # test: base, sanitize, error returns
	...
	# test: base, transaction and error returns (not found, validation error, and bad attr error)
	def find_and_update_all(attributes_array)
		...
		return updated_models, success, e
	end
	...
	def find_by_id(id)
		...
		error = ActiveRecord::RecordNotFound unless success
	...
	def update(model, attributes)
		...
		begin
			...
			success = false
			error = ActiveModel::MassAssignmentSecurity::Error
		end
